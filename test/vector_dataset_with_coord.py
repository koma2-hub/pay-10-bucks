...
for i, file in enumerate(file_names1):
    ...
    for j in range(8):
        ...
        #近傍点の座標と輝度値を取得する
        pcd_positive = np.copy(pcd[neighbor_indices])
        #取得した点群にノイズを加えたものを生成
        noise = np.zeros((k, 4))
        noise[:, -1] += np.random.normal(0, 0.05, k)
        pcd_positive += noise
        #正例としてデータに追加
        dummy_pairs.append((pcd[neighbor_indices], pcd_positive, 1))
    """
    負例のペアの作成
    """
    ...
    for j in range(8):
        ...
        #サンプリングする点とその近傍点を取得
        pcd_negative = negative_pcd[neighbor_indices_negative]
        
        #ヒストグラムを負のペアとして保存
        dummy_pairs.append((pcd[neighbor_indices], pcd_negative, 0))
...
for i, file in enumerate(file_names2):
    ...
    for j in range(8):
        ...
        pcdnegative = negative_pcd[neighbor_indices_negative]
...
# .npz 形式で圧縮して保存
save_file = 'vector_dataset_'  + str(k) + 'points_' +'noise005_'+ str(intensity1_list.shape[0]) + '_labs_with_coord' + '.npz'
