...
def main():
    ...
    for i in range(args.num_vis):
        ...
        predict_src_xyz = torch.matmul(rotation_ab_pred, src_xyz) + translation_ab_pred.unsqueeze(2)
        ...
        visualize_pcd(predict_src_xyz, transformed_src_true)

        #print("お試し 回転の予測値と並進の真値で位置合わせ")
        otamesi = src[:, :3, :].clone().detach()
        otamesi_trans = torch.matmul(rotation_ab_pred, otamesi) + t_gt
        visualize_pcd(otamesi_trans, transformed_src_true)
        print("回転行列の真値", R_gt)
        print("並進の真値",t_gt)

        print("回転行列の予測値", rotation_ab_pred)
        print("並進の予測値", translation_ab_pred.unsqueeze(2))

        print("回転行列の損失", F.mse_loss(torch.matmul(rotation_ab_pred.transpose(2,1), R_gt), torch.eye(3).unsqueeze(0)))
        print("並進の損失", F.mse_loss(t_gt, translation_ab_pred.unsqueeze(2)))
