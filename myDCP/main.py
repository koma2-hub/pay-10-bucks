...
try:
    ...
    print("この main.py と同じディレクトリに、DCPDataset クラスを含む")
...
def main():
    ...
    if train_size == 0 or test_size == 0:
        ...
        train_dataset = all_data_dataset
        test_dataset = all_data_dataset
    else:
        print(f"Splitting data ({total_size} pairs) into train: {train_size}, test: {test_size}")
        # 3. random_split を使ってデータセットを2つに分割
        #    (seed を固定することで、毎回同じように分割されます)
